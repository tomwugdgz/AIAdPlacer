...
# ── 日志 ────────────────────────────────────────────────────────────────────────
logger = setup_logging("tom_agent", log_file="logs/tom_agent.log")
logger.info("Tom Agent 启动中...")
...
- roi_calculate: ROI 计算（在生成投放方案后自动返回三场景 ROI，无需手动调用）
"""
...
class ChatRequest(BaseModel):
    ...
    use_mcp: bool = Field(True, description="是否调用 MCP 工具")

class ChatResponse(BaseModel):
    """聊天响应"""
    role: str = Field(default="assistant", description="响应角色")
    content: str = Field(..., description="响应内容")
    mcp_calls: Optional[List[Dict[str, Any]]] = Field(default=None, description="MCP 工具调用记录")
...
class PlanGenerateRequest(BaseModel):
    ...
    cities: List[str] = Field(..., description="投放城市列表")
    target_audience: Optional[str] = Field(default=None, description="目标受众")
    goals: Optional[List[str]] = Field(default=None, description="投放目标")
...
class PlanGenerateResponse(BaseModel):
    ...
    media_mix: List[Dict[str, Any]] = Field(..., description="媒体组合")
    estimated_reach: int = Field(..., description="预估触达人数")
    estimated_impressions: int = Field(..., description="预估曝光次数")
    total_cost: float = Field(..., description="总成本")
    timeline: List[Dict[str, str]] = Field(..., description="时间线")
    roi_result: Optional[Dict[str, Any]] = Field(default=None, description="ROI 计算结果")
    roi_visualization: Optional[str] = Field(default=None, description="ROI 可视化图表 HTML")

class CpmTrackRequest(BaseModel):
    """CPM 追踪请求"""
    campaign_id: str = Field(..., description="投放计划 ID")
...
class CpmTrackResponse(BaseModel):
    ...
    impressions: int = Field(..., description="曝光次数")
    clicks: int = Field(..., description="点击次数")
    ctr: float = Field(..., description="点击率")
    cpm: float = Field(..., description="千次曝光成本")
    details: List[Dict[str, Any]] = Field(..., description="详细数据")

class CpmCompareRequest(BaseModel):
    """CPM 对比请求"""
    campaigns: List[str] = Field(..., description="投放计划 ID 列表")

class CpmCompareResponse(BaseModel):
    """CPM 对比响应"""
    campaigns: List[Dict[str, Any]] = Field(..., description="投放计划列表")
...
@cached(ttl=60, maxsize=50)  # 缓存 1 分钟，最多 50 条
def call_mcp_tool(tool_name: str, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.debug(f"调用 MCP 工具: {tool_name}, params={params}")
...
async def call_roi_agent(cost: float, cities: List[str], product: str = None) -> Optional[Dict]:
    ...
    Returns:
        Optional[Dict]: ROI 计算结果（如果调用失败则返回 None）
    """
    request_id = generate_request_id("call_roi")
    try:
        # 根据城市数量调整 N（广告框数）
        ...
        # 使用 httpx 的 params 参数，避免 URL 编码问题
        params = {
            "N": N,
            "cost": int(cost),
            "T": 14
        ...
        if cities and len(cities) > 0:
            params["city"] = cities[0]  # 取第一个城市
        if product:
            params["product"] = product
        
        logger.info(f"调用 ROI Agent [{request_id}]: cost={cost}, cities={cities}, product={product}")
        
        async with httpx.AsyncClient(timeout=10.0) as client:
            resp = await client.get("http://127.0.0.1:5004/api/v2/roi/three-scenarios", params=params)
            if resp.status_code == 200:
                data = resp.json()
                logger.info(f"ROI Agent 调用成功 [{request_id}]: cost={cost}, roi_pessimistic={data.get('scenarios', {}).get('pessimistic', {}).get('roi_percent', 'N/A')}")
                ...
                logger.warning(f"ROI Agent 返回非 200 [{request_id}]: {resp.status_code}, {resp.text[:200]}")
                return None
    except Exception as e:
        logger.warning(f"ROI Agent 调用失败（已忽略，不影响主流程）[{request_id}]: {e}")
        import traceback
        logger.warning(f"详细错误 [{request_id}]: {traceback.format_exc()}")
        return None
...
async def call_llm(messages: List[Dict], stream: bool = True):
    ...
    Returns:
        流式时返回 StreamingResponse，非流式时返回字符串
        
    Raises:
        HTTPException: LLM 调用失败
    """
    import os
    import openai
...
async def health_check():
    ...
    return {"status": "ok", "agent": "Tom", "version": "2.2.0"}
...
async def chat(req: ChatRequest):
    ...
    Raises:
        HTTPException: 参数验证失败或 LLM 调用错误
    """
    request_id = generate_request_id("chat")
    logger.info(f"收到聊天请求 [{request_id}]: stream={req.stream}, use_mcp={req.use_mcp}")
    
    try:
        # 参数验证
        if not req.messages or len(req.messages) == 0:
            raise ValidationError(message="messages 参数必须非空", details={"messages": req.messages})
        ...
        logger.info(f"聊天请求处理完成 [{request_id}]")
        return response
        ...
        logger.error(f"聊天请求失败 [{request_id}]: {str(e)}", exc_info=True)
...
async def chat_stream(req: ChatRequest):
    ...
    Raises:
        HTTPException: 参数验证失败或 LLM 调用错误
    """
    request_id = generate_request_id("chat_stream")
    logger.info(f"收到流式聊天请求 [{request_id}]: use_mcp={req.use_mcp}")
    ...
    try:
        ...
        async def generate():
            try:
                ...
                logger.error(f"流式响应失败 [{request_id}]: {e}", exc_info=True)
                yield f"data: {json.dumps({'error': str(e)}, ensure_ascii=False)}\n\n"
        
        logger.info(f"流式聊天请求处理完成 [{request_id}]")
        ...
        logger.error(f"流式聊天请求失败 [{request_id}]: {str(e)}", exc_info=True)
        return JSONResponse(status_code=500, content=format_error_response(e, request_id=request_id))
...
async def generate_plan(req: PlanGenerateRequest):
    ...
    Raises:
        HTTPException: 参数验证失败或方案生成错误
    """
    request_id = generate_request_id("plan")
    logger.info(f"收到投放方案生成请求 [{request_id}]: brand={req.brand}, product={req.product}, budget={req.budget}, cities={req.cities}")
    
    try:
        # 参数验证
        if not req.brand or not isinstance(req.brand, str):
            raise ValidationError(message="brand 参数必须为非空字符串", details={"brand": req.brand})
        if not req.product or not isinstance(req.product, str):
            raise ValidationError(message="product 参数必须为非空字符串", details={"product": req.product})
        if req.budget <= 0:
            raise ValidationError(message="budget 必须大于 0", details={"budget": req.budget})
        if not req.cities or len(req.cities) == 0:
            raise ValidationError(message="cities 参数必须非空", details={"cities": req.cities})
        ...
        logger.info(f"投放方案生成请求处理完成 [{request_id}]: plan_id={plan_id}")
        return response
        ...
        logger.error(f"投放方案生成请求失败 [{request_id}]: {str(e)}", exc_info=True)
        return JSONResponse(status_code=500, content=format_error_response(e, request_id=request_id))

@router.post("/cpm/track", response_model=Dict[str, Any])
@monitor_performance_async(logger=logger)
async def cpm_track(req: CpmTrackRequest):
    ...
    Returns:
        Dict[str, Any]: CPM 追踪结果
        
    Raises:
        HTTPException: 参数验证失败或查询错误
    """
    request_id = generate_request_id("cpm_track")
    logger.info(f"收到 CPM 追踪请求 [{request_id}]: campaign_id={req.campaign_id}, date={req.date}")
    
    try:
        # 参数验证
        if not req.campaign_id or not isinstance(req.campaign_id, str):
            raise ValidationError(message="campaign_id 参数必须为非空字符串", details={"campaign_id": req.campaign_id})
        ...
        logger.info(f"CPM 追踪请求处理完成 [{request_id}]")
        return response
        ...
        logger.error(f"CPM 追踪请求失败 [{request_id}]: {str(e)}", exc_info=True)
        return JSONResponse(status_code=500, content=format_error_response(e, request_id=request_id))

@router.post("/cpm/compare", response_model=Dict[str, Any])
@monitor_performance_async(logger=logger)
async def cpm_compare(req: CpmCompareRequest):
    ...
    Returns:
        Dict[str, Any]: CPM 对比结果
        
    Raises:
        HTTPException: 参数验证失败或查询错误
    """
    request_id = generate_request_id("cpm_compare")
    logger.info(f"收到 CPM 对比请求 [{request_id}]: campaigns={req.campaigns}")
    
    try:
        # 参数验证
        if not req.campaigns or len(req.campaigns) == 0:
            raise ValidationError(message="campaigns 参数必须非空", details={"campaigns": req.campaigns})
        ...
        logger.info(f"CPM 对比请求处理完成 [{request_id}]")
        return response
        ...
        logger.error(f"CPM 对比请求失败 [{request_id}]: {str(e)}", exc_info=True)
        return JSONResponse(status_code=500, content=format_error_response(e, request_id=request_id))

@router.post("/query/points", response_model=Dict[str, Any])
@monitor_performance_async(logger=logger)
async def query_points(req: QueryPointsRequest):
    ...
    Raises:
        HTTPException: 参数验证失败或查询错误
    """
    request_id = generate_request_id("query_points")
    logger.info(f"收到点位查询请求 [{request_id}]: query={req.query}")
    
    try:
        # 参数验证
        if not req.query or not isinstance(req.query, str):
            raise ValidationError(message="query 参数必须为非空字符串", details={"query": req.query})
        ...
        logger.info(f"点位查询请求处理完成 [{request_id}]: results_count={len(results)}")
        return response
        ...
        logger.error(f"点位查询请求失败 [{request_id}]: {str(e)}", exc_info=True)
        return JSONResponse(status_code=500, content=format_error_response(e, request_id=request_id))

@router.get("/tools")
async def list_tools():
    """列出 Tom Agent 可用的 MCP 工具"""
    request_id = generate_request_id("list_tools")
    logger.info(f"收到工具列表请求 [{request_id}]")
    ...
    try:
        ...
        logger.info(f"工具列表请求处理完成 [{request_id}]")
        return response
        
    except Exception as e:
        logger.error(f"工具列表请求失败 [{request_id}]: {str(e)}", exc_info=True)
        return JSONResponse(status_code=500, content=format_error_response(e, request_id=request_id))
